...
RELEASE_DIR  = $(CURDIR)/_build/default/rel/enls
VERSION     := $(shell erl -noshell -eval 'io:format("~s", [case file:consult("$(CURDIR)/src/enls.app.src") of {ok, [{_, _, Terms}]} -> proplists:get_value(vsn, Terms, "0.0.0"); _ -> "0.0.0" end]),erlang:halt().')
RELEASE_NAME = enls-$(VERSION)
NAME_UPPER  := $(shell echo enls | awk '{print toupper($$1)}')
...
compile:
	...
	$(PRE) \
            ...
            $(REBAR) compile \
        $(POST)
	$(PRE) cp -r $(CURDIR)/_build/default/lib/enls/ebin $(CURDIR)
...
shell:
	...
	$(PRE) \
        ...
        erl     -pa `ls -d _build/default/lib/*/ebin` \
                ...
                -eval "begin application:load('enls'), catch code:load_file('enls') end" \
                +B
